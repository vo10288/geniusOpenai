...
class AIAssistantWithGenie:
    ...
    def speak_text(self, text):
        ...
        def speak():
            ...
            try:
                ...
                print(f"Errore nel pronunciare: {text}")
            finally:
                self.is_speaking = False
            ...
            while True:
                ...
                if ret:
                    ...
                    if (faces_detected and 
                        ...
                        # Aspetta un po' e poi inizia ad ascoltare
                        threading.Timer(3.0, lambda: self.handle_listening()).start()
    ...
    def handle_listening(self):
        """Gestisce l'ascolto automatico dopo una domanda"""
        if self.face_detected and not self.is_speaking:
            user_question = self.listen_to_user()
            if user_question:
                response = self.get_openai_response(user_question)
                print(f"\n🤖 OpenAI: {response}")
...
def main():
    ...
    # resto del codice...
#def main():
    # Richiede la chiave API di OpenAI
#    api_key = input("🔑 Inserisci la tua OpenAI API key: ").strip()
    ...
    try:
        ...
        print("\n🚀 Avvio dell'AI Assistant con Genio...")
        print("💡 Suggerimenti:")
